...
do
    for size in "width=3000&height=1800" "width=1000&height=500" "width=1500&height=1000"
    do
        mkdir -p "png/$size/$theme/node-exporter"
        mkdir -p "png/$size/$theme/k8s"
        i=1
        # replace by IP array, example: 'for worker in "10.1.20.181" "10.1.20.46" "10.1.20.190"'
        ...
        do
            ...
            curl -s -o png/$size/$theme/node-exporter/network-worker-$i.png             "$base_url/rYdddlPWk/node-exporter-full?orgId=1&from=$start&to=$end&var-job=node-exporter&var-node=$worker:9100&var-diskdevices=%5Ba-z%5D%2B%7Cnvme%5B0-9%5D%2Bn%5B0-9%5D%2B&theme=$theme&panelId=74&$size&$tz"
        ...
        curl -s -o png/$size/$theme/k8s/k8s-cpu-by-namespaces.png                       "$base_url/efa86fd1d0c121a26444b636a3f509a8/kubernetes-compute-resources-cluster?orgId=1&refresh=10s&from=$start&to=$end&theme=$theme&panelId=7&$size&$tz"                                                       &\
        curl -s -o png/$size/$theme/k8s/k8s-memory-by-namespaces.png                    "$base_url/efa86fd1d0c121a26444b636a3f509a8/kubernetes-compute-resources-cluster?orgId=1&refresh=10s&from=$start&to=$end&theme=$theme&panelId=9&$size&$tz"                                                       &\
        curl -s -o png/$size/$theme/k8s/k8s-cpu-by-pods-in-default-namespace.png        "$base_url/85a562078cdf77779eaa1add43ccec1e/kubernetes-compute-resources-namespace-pods?orgId=1&refresh=10s&from=$start&to=$end&theme=$theme&panelId=5&$size&$tz"                                                &\
        curl -s -o png/$size/$theme/k8s/k8s-memory-by-pods-in-default-namespace.png     "$base_url/85a562078cdf77779eaa1add43ccec1e/kubernetes-compute-resources-namespace-pods?orgId=1&refresh=10s&from=$start&to=$end&theme=$theme&panelId=7&$size&$tz"                                                &\
        # warning: 'var-node=worker-1&var-node=worker-2&var-node=worker-3' block is variable
        curl -s -o png/$size/$theme/k8s/k8s-cpu-by-all-pods.png                         "$base_url/200ac8fdbfbb74b39aff88118e4d1c2c/kubernetes-compute-resources-node-pods?orgId=1&refresh=10s&var-datasource=default&var-cluster=&var-node=worker-1&var-node=worker-2&var-node=worker-3&from=$start&to=$end&theme=$theme&panelId=1&$size&$tz"
        curl -s -o png/$size/$theme/k8s/k8s-memory-by-all-pods.png                      "$base_url/200ac8fdbfbb74b39aff88118e4d1c2c/kubernetes-compute-resources-node-pods?orgId=1&refresh=10s&var-datasource=default&var-cluster=&var-node=worker-1&var-node=worker-2&var-node=worker-3&from=$start&to=$end&theme=$theme&panelId=3&$size&$tz"          &\
        curl -s -o png/$size/$theme/k8s/k8s-cpu-by-sts.png                              "$base_url/a164a7f0339f99e89cea5cb47e9be617/kubernetes-compute-resources-workload?orgId=1&refresh=10s&from=$start&to=$end&theme=$theme&panelId=1&$size&$tz"                                                                 &\
        curl -s -o png/$size/$theme/k8s/k8s-memory-by-sts.png                           "$base_url/a164a7f0339f99e89cea5cb47e9be617/kubernetes-compute-resources-workload?orgId=1&refresh=10s&from=$start&to=$end&theme=$theme&panelId=3&$size&$tz"                                                                 &\
        curl -s -o png/$size/$theme/k8s/k8s-net-receive-by-namespaces.png               "$base_url/ff635a025bcfea7bc3dd4f508990a3e9/kubernetes-networking-cluster?orgId=1&refresh=10s&var-resolution=30s&var-interval=4h&var-datasource=default&var-cluster=&from=$start&to=$end&theme=$theme&panelId=10&$size&$tz" &\
        curl -s -o png/$size/$theme/k8s/k8s-net-transmit-by-namespaces.png              "$base_url/ff635a025bcfea7bc3dd4f508990a3e9/kubernetes-networking-cluster?orgId=1&refresh=10s&var-resolution=30s&var-interval=4h&var-datasource=default&var-cluster=&from=$start&to=$end&theme=$theme&panelId=11&$size&$tz"
        curl -s -o png/$size/$theme/k8s/k8s-net-receive-by-all-pods.png                 "$base_url/8b7a8b326d7a6f1f04244066368c67af/kubernetes-networking-namespace-pods?orgId=1&refresh=10s&var-datasource=default&var-cluster=&var-namespace=All&var-resolution=30s&var-interval=4h&from=$start&to=$end&theme=$theme&panelId=7&$size&$tz"                               &\
        curl -s -o png/$size/$theme/k8s/k8s-net-transmit-by-all-pods.png                "$base_url/8b7a8b326d7a6f1f04244066368c67af/kubernetes-networking-namespace-pods?orgId=1&refresh=10s&var-datasource=default&var-cluster=&var-namespace=All&var-resolution=30s&var-interval=4h&from=$start&to=$end&theme=$theme&panelId=8&$size&$tz"                               &\
        curl -s -o png/$size/$theme/k8s/k8s-net-receive-by-sts.png                      "$base_url/bbb2a765a623ae38130206c7d94a160f/kubernetes-networking-namespace-workload?orgId=1&refresh=10s&var-datasource=default&var-cluster=&var-namespace=default&var-type=statefulset&var-resolution=30s&var-interval=4h&from=$start&to=$end&theme=$theme&panelId=10&$size&$tz" &\
